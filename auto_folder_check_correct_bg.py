...
CORE_REPOS_STRUCTURE = {
    "vision_cortex": FolderStructure(
        name="vision_cortex",
        required_dirs=[
            "src", "src/core", "src/modules", "src/utils",
            "tests", "tests/unit", "tests/integration",
            "docs", ".github", ".github/workflows"
        ],
        required_files=["pyproject.toml", "requirements.txt", "README.md", ".gitignore"]
    ),
    ...
    "index": FolderStructure(
        ...
        required_files=[
            ...
            "config/repos.yaml", "config/workspace.yaml"
        ]
    ),
}
...
class FolderChecker:
    ...
    def __init__(self, logger: logging.Logger, project_root: Path, router: AutoFolderRouter):
        self.logger = logger
        self.project_root = project_root
        self.router = router
    ...
    def check_all_repos(self) -> Dict[str, CheckResult]:
        ...
        for repo_name, structure in CORE_REPOS_STRUCTURE.items():
            ...
            if repo_path.exists():
                ...
                results[repo_name] = CheckResult(repo_name=repo_name, is_valid=False, issues=1)
...
class FolderCorrector:
    ...
    def _create_file_template(self, repo_path: Path, file_name: str, repo_name: str) -> None:
...
def run_background_check():
    ...
    logger = setup_logging(str(project_root / "vision_cortex" / "auto_folder_check.log"))
