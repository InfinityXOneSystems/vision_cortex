...
export class LLMEntityResolver extends EventEmitter {
  ...
  private matchByName(signal: Signal): ResolutionMatch | null {
    ...
    for (const [entityId, entity] of this.entities) {
      ...
      if (nameScore >= 0.85) {
        candidates.push({ entityId, matchScore: nameScore, matchReasons: [`Fuzzy name match: \"${signal.entity.name}\" ≈ \"${entity.canonicalName}\"`] });
      }
      for (const alias of entity.aliases) {
        const aliasScore = this.fuzzyMatchScore(signal.entity.name, alias);
        if (aliasScore >= 0.85 && aliasScore > nameScore) {
          candidates.push({ entityId, matchScore: aliasScore, matchReasons: [`Alias match: \"${signal.entity.name}\" ≈ \"${alias}\"`] });
        }
      }
    }
  ...
  /**
   * Get timeline (signals) for an entity
   */
  getEntityTimeline(entityId: string): Signal[] {
    const entity = this.entities.get(entityId);
    return entity ? entity.signals : [];
    ...
    try {
      const result = await this.ollama.generateJSON<LLMResolutionResult>(
        import { EventEmitter } from "events";
        import { OllamaClient } from "../utils/ollama-client";
        import type { Signal } from "./scoring-engine";
    ...
    for (const [key, value] of Object.entries(identifiers)) {
      ...
      if (entityId) {
        return {
          entityId,
          matchScore: 0.99, // Identifier match is near-certain
          matchReasons: [`Identifier match: ${key} = ${value}`],
        };
      }
    }
  ...
  /**
   * Rules-based fuzzy name matching (Levenshtein distance, soundex, etc.)
   */
  private matchByName(signal: Signal): ResolutionMatch | null {
    const candidates: ResolutionMatch[] = [];

    for (const [entityId, entity] of this.entities) {
      const nameScore = this.fuzzyMatchScore(
        signal.entity.name,
      ...
      if (nameScore >= 0.85) {
        candidates.push({
          ...
          matchReasons: [
            `Fuzzy name match: "${signal.entity.name}" ≈ "${entity.canonicalName}"`,
          ],
        });
      }
      ...
      for (const alias of entity.aliases) {
        ...
        if (aliasScore >= 0.85 && aliasScore > nameScore) {
          candidates.push({
            ...
            matchReasons: [`Alias match: "${signal.entity.name}" ≈ "${alias}"`],
          });
        }
      }
    }
  ...
  /**
   * Levenshtein distance-based fuzzy matching (0-1 score)
   */
  private fuzzyMatchScore(str1: string, str2: string): number {
    const s1 = str1.toLowerCase().trim();
    const s2 = str2.toLowerCase().trim();
  ...
  private addSignalToEntity(entityId: string, signal: Signal): Entity {
    ...
    this.emit("resolver:signal-added", {
      entityId,
  ...
  private createEntity(signal: Signal): Entity {
    ...
    // Index identifiers
    for (const [_key, value] of Object.entries(entity.identifiers)) {
      if (value) {
        this.identifierIndex.set(value, entityId);
      }
    }

    this.emit("resolver:entity-created", {
      entityId,
