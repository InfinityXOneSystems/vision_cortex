...
class InteractiveTodoSystem:
    ...
    def _save_todos(self):
        ...
        try:
            with open(self.todos_file, "w", encoding="utf-8") as f:
                json.dump({k: asdict(v) for k, v in self.todos.items()}, f, indent=2)
        except Exception as e:
    ...
    def get_todos_by_status(self, status: str) -> List[Dict[str, Any]]:
        """Get todos by status"""
        todos = [asdict(t) for t in self.todos.values() if t.status == status]
    ...
    def get_todos_by_assignee(self, assignee: str) -> List[Dict[str, Any]]:
        ...
        return [asdict(t) for t in self.todos.values() if t.assigned_to == assignee]

    def get_todos_by_tag(self, tag: str) -> List[Dict[str, Any]]:
        """Get todos by tag"""
        return [asdict(t) for t in self.todos.values() if tag in t.tags]

    def get_todos_by_priority(self, priority: int) -> List[Dict[str, Any]]:
        """Get todos by priority"""
        todos = [asdict(t) for t in self.todos.values() if t.priority == priority]
    ...
    def get_overdue_todos(self) -> List[Dict[str, Any]]:
        ...
        overdue = [
            ...
            if t.due_date
            and t.due_date < now
            and t.status != TodoStatus.COMPLETED.value
        ]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        completed = sum(
            1 for t in self.todos.values() if t.status == TodoStatus.COMPLETED.value
        )
        ...
        return {
            ...
            "by_status": statuses,
            "completed_percent": (
                round(completed / len(self.todos) * 100, 1) if self.todos else 0
            ),
            "total_estimated_hours": round(total_estimated, 2),
            "total_actual_hours": round(total_actual, 2),
            "avg_estimated_hours": (
                round(total_estimated / len(self.todos), 2) if self.todos else 0
            ),
            "high_priority_count": sum(
                1 for t in self.todos.values() if t.priority >= TodoPriority.HIGH.value
            ),
        }
    ...
    def list_todos(
        self, filter_status: Optional[str] = None, sort_by: str = "priority"
    ) -> List[Dict[str, Any]]:
