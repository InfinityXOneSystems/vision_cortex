...
class DocSyncSystem:
        ...
        if self.locations_file.exists():
            try:
                with open(self.locations_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    return {k: SyncLocation(**v) for k, v in data.items()}
    ...
    def _save_locations(self):
        ...
        try:
            with open(self.locations_file, "w", encoding="utf-8") as f:
                json.dump(
                    {k: asdict(v) for k, v in self.locations.items()}, f, indent=2
                )
        except Exception as e:
    ...
    def _save_sync_log(self):
        ...
        try:
            with open(self.sync_log_file, "w", encoding="utf-8") as f:
                json.dump([asdict(r) for r in self.sync_records], f, indent=2)
        except Exception as e:
            logger.error(f"Failed to save sync log: {e}")

    def add_location(
        self, location_id: str, name: str, path: str, location_type: str = "local"
    ) -> bool:
        """
        Add sync location
    ...
    def _get_location_files(
        self, location_id: str, pattern: str = "*.md"
    ) -> List[Path]:
    ...
    def get_sync_history(
        self, location_id: Optional[str] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """Get sync history"""
