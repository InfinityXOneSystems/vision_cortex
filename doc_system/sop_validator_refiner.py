...
class SOPValidator:
    ...
    def validate_sop_comprehensive(
        self, sop_data: Dict, execution_results: Dict = None
    ) -> ValidationReport:
        ...
        self.logger.info(
            f"{status} - Quality: {quality_score:.1f}% ({errors}E, {warnings}W, {suggestions}S)"
        )
    ...
    def _check_execution(
        self, sop_data: Dict, execution_results: Dict
    ...
    def _generate_recommendations(self, findings: List[ValidationFinding]) -> List[str]:
        ...
        if errors:
            recommendations.append(
                f"Fix {len(errors)} critical errors before using SOP"
            )

        warnings = [f for f in findings if f.finding_type == "warning"]
        if warnings:
            recommendations.append(
                f"Review {len(warnings)} warnings for quality improvement"
            )

        suggestions = [f for f in findings if f.finding_type == "suggestion"]
        if suggestions:
            recommendations.append(
                f"Consider {len(suggestions)} suggestions for optimization"
            )
...
class SOPRefiner:
    ...
    def refine_from_validation(
        self, sop_data: Dict, validation_report: ValidationReport
    ) -> Dict:
    ...
    def _apply_auto_fix(
        self, sop_data: Dict, finding: ValidationFinding
    ) -> Tuple[bool, Dict]:
                ...
                improvements.append(
                    {
                        ...
                        "priority": (
                            ...
                            else "medium" if finding.severity >= 2 else "low"
                        ),
                    }
                )
