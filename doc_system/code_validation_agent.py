...
class CodeValidationAgent:
    ...
    def __init__(
        self, workspace_root: str = ".", index_system=None, config: Dict = None
    ):
        ...
        self.required_docstring_types = self.config.get(
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                if ".then(" in line or ".catch(" not in "\n".join(
                    ...
                    if ".catch" not in "\n".join(
                        lines[max(0, i - 1) : min(i + 3, len(lines))]
                    ):
                        results.append(
                            ValidationResult(
                                check_type="code_quality",
    ...
    def validate_directory(
        ...
        for pattern in file_patterns:
            for file_path in dir_path.glob(pattern):
                ...
                success, message = self.validate_file(
                    str(file_path), roadmap_item_id, todo_id
                )
                validated += 1
                issues += len(
                    [
    ...
    def generate_commit_message(
        self, roadmap_item_id: str = None, todo_id: str = None
    ) -> str:
        """
        ...
        try:
            if file_paths:
                for file_path in file_paths:
                    subprocess.run(
                        ["git", "add", file_path], check=True, cwd=self.workspace_root
                    )
            else:
                subprocess.run(
                    ["git", "add", "-A"], check=True, cwd=self.workspace_root
                )
    ...
    def git_push(
        self, branch: str = "main", delay_seconds: int = 2
    ) -> Tuple[bool, str]:
