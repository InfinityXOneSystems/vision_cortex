...
class AgentConfigurationLoader:
    ...
    def _get_agent_constraints(self, agent_name: str) -> Dict[str, Any]:
        ...
        if agent_name == "code_validation":
            constraints = {
                ...
                "timeout_seconds": self.config.get("defaults", {}).get("timeout_seconds", 300),
            ...
            constraints = {
                "timeout_seconds": self.config.get("defaults", {}).get("timeout_seconds", 300),
                "require_validation": self.config.get("git", {}).get("require_validation", True),
                "auto_push": self.config.get("git", {}).get("auto_push", True),
                "commit_format": self.config.get("git", {}).get("commit_format", "conventional"),
                "max_retries": 3
            }
    ...
    def validate_agent_setup(self, agent_name: str) -> Tuple[bool, List[str]]:
        ...
        system_languages = [lang for lang_config in self.config.get("code", {}).values() 
    ...
    def get_mandatory_requirements(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "auto_push": self.config.get("git", {}).get("auto_push", True),
            "validation_required": self.config.get("git", {}).get("require_validation", True),
    ...
    def validate_environment(self) -> Tuple[bool, List[str]]:
        ...
        python_req = self.config.get("code", {}).get("python", {}).get("version", "3.8+")
        import sys
