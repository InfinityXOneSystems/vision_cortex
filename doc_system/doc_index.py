...
class DocIndexSystem:
    ...
    def __init__(self, data_dir: str = "doc_system/data"):
        ...
        self.todos: Dict[str, LinkedTodo] = self._load_json(
            self.todos_index_path, {}
        )
    ...
    def _update_search_index(
        ...
        searchable = f"{name} {description} {' '.join(tags)} {' '.join(keywords)}".lower()
        ...
        for doc_id, doc in self.documents.items():
            ...
            if match_count > 0:
                results.append({
                    ...
                    'tags': doc.get('tags', []),
                    'match_score': match_count / len(query_lower) if query_lower else 0
                })
    ...
    def get_roadmap_progress(self) -> Dict:
        ...
        completed = sum(1 for item in self.roadmap.values() if item.get('status') == 'completed')
        in_progress = sum(1 for item in self.roadmap.values() if item.get('status') == 'in-progress')
        planned = sum(1 for item in self.roadmap.values() if item.get('status') == 'planned')
        ...
        total_estimated = sum(item.get('estimated_effort', 0) for item in self.roadmap.values())
        total_actual = sum(item.get('actual_effort', 0) for item in self.roadmap.values())
    ...
    def get_todo_progress(self) -> Dict:
        ...
        completed = sum(1 for todo in self.todos.values() if todo.get('status') == 'completed')
        in_progress = sum(1 for todo in self.todos.values() if todo.get('status') == 'in-progress')
        blocked = sum(1 for todo in self.todos.values() if todo.get('status') == 'blocked')
        ...
        # Time metrics
        total_estimated = sum(todo.get('estimated_hours', 0) for todo in self.todos.values())
        total_actual = sum(todo.get('actual_hours', 0) for todo in self.todos.values())
        
        # Overdue
        now = datetime.now().isoformat()
        overdue = sum(1 for todo in self.todos.values() 
                     if todo.get('due_date') and todo.get('due_date') < now 
    ...
    def _count_by_key(self, collection: str, key: str) -> Dict:
        ...
        counts = {}
        items = self.documents if collection == 'documents' else (
            self.roadmap if collection == 'roadmap' else self.todos
        )
...
"""
    ...
    def _format_section_table(self, data: Dict) -> str:
        ...
        lines = ["| Section | Planned | In Progress | Completed |", "|---------|---------|-------------|-----------|"]
        for section in sorted(data.keys()):
            counts = data[section]
            lines.append(f"| {section} | {counts['planned']} | {counts['in_progress']} | {counts['completed']} |")
    ...
    def _format_priority_table(self, data: Dict) -> str:
        ...
        for priority in sorted(data.keys()):
            lines.append(f"| {priority_names.get(priority, 'Unknown')} | {data[priority]} |")
