...
class AutoSOPAgent:
    ...
    def __init__(self, repo_root: str = ".", sop_dir: str = None):
        ...
        self.sop_dir = (
            Path(sop_dir) if sop_dir else Path(repo_root) / "doc_system" / "sops"
        )
                ...
                try:
                    ...
                    sop = SOPDefinition(
                        ...
                        validation_level=ValidationLevel[
                            sop_data.get("validation_level", "PERMISSIVE")
                        ],
    ...
    def create_sop_from_operations(
        ...
        try:
            ...
            self._log_operation(
                "create_sop", sop_id, f"Created with {len(steps)} steps"
            )
    ...
    def execute_sop(
        self, sop_id: str, dry_run: bool = False
        ...
        execution.status = (
            StepStatus.COMPLETED if len(failed_steps) == 0 else StepStatus.FAILED
        )
        ...
        total_duration = (
            sop.average_duration * (sop.execution_count - 1)
        ...
        status_str = (
            "✓ COMPLETED" if execution.status == StepStatus.COMPLETED else "✗ FAILED"
        )
        self.logger.info(
            f"{status_str} - {execution.steps_completed}/{execution.steps_total} steps ({execution.success_rate:.1f}%)"
        )

        self._log_operation(
            "execute_sop",
            sop_id,
            f"{execution.status.value} - {execution.success_rate:.1f}% success",
        )
    ...
    def validate_sop(
        self, sop_id: str, validator_agent: any = None
    ) -> Tuple[bool, Dict]:
    ...
    def refine_sop(
        self, sop_id: str, updates: Dict, refined_by: str = "validator"
    ) -> Tuple[bool, str]:
    ...
    def export_sop_report(self, sop_id: str, filepath: str) -> bool:
        ...
        try:
            ...
            report = {
                "sop": asdict(sop),
                "execution_history": [
                    asdict(e) for e in self.execution_history if e.sop_id == sop_id
                ],
                "metrics": {
                    ...
                    "success_rate": (
                        (sop.success_count / sop.execution_count * 100)
